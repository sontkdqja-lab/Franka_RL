...
"""Play a vision-based transformer student policy in Isaac Lab."""
...
parser = argparse.ArgumentParser(description="Play a transformer student policy.")
...
# student policy always requires camera sensors
...
def capture_student_inputs(env) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    proprio = torch.cat([robot.data.joint_pos, robot.data.joint_vel, env.action_manager.action], dim=-1).clone()
    return main_rgb, sub_rgb, proprio


def initialize_history(main_rgb: torch.Tensor, sub_rgb: torch.Tensor, proprio: torch.Tensor, seq_len: int):
    main_hist = main_rgb.unsqueeze(1).repeat(1, seq_len, 1, 1, 1)
    sub_hist = sub_rgb.unsqueeze(1).repeat(1, seq_len, 1, 1, 1)
    proprio_hist = proprio.unsqueeze(1).repeat(1, seq_len, 1)
    return main_hist, sub_hist, proprio_hist
...
def update_history(
    ...
    if torch.any(done_mask):
        ...
        proprio_hist[done_mask] = proprio[done_mask].unsqueeze(1).repeat(1, proprio_hist.shape[1], 1)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    main_rgb, sub_rgb, proprio = capture_student_inputs(env.unwrapped)
    main_hist, sub_hist, proprio_hist = initialize_history(main_rgb, sub_rgb, proprio, sequence_length)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            actions = student_policy(main_hist, sub_hist, proprio_hist)
            obs, _, dones, _ = env.step(actions)

        terminated = env.unwrapped.reset_terminated.clone()
        time_outs = env.unwrapped.reset_time_outs.clone()
        main_rgb, sub_rgb, proprio = capture_student_inputs(env.unwrapped)
        main_hist, sub_hist, proprio_hist = update_history(
            main_hist, sub_hist, proprio_hist, main_rgb, sub_rgb, proprio, dones
        )

        if torch.any(dones):
            episode_count += int(dones.sum().item())
            success_mask = terminated & ~time_outs
            success_count += int(success_mask.sum().item())
            print(
                f"[INFO] Episodes={episode_count}, successes={success_count}, "
                f"latest_successes={int(success_mask.sum().item())}"
            )
