...
def release_separation_reward(
    env: ManagerBasedRLEnv,
    xy_threshold: float = 0.05,
    height_threshold: float = 0.08,
    min_open_fraction: float = 0.60,
    separation_threshold: float = 0.06,
    command_name: str = "drop_pose",
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    object_cfg: SceneEntityCfg = SceneEntityCfg("object"),
    ee_frame_cfg: SceneEntityCfg = SceneEntityCfg("ee_frame"),
) -> torch.Tensor:
    """Reward releasing the object and separating the end-effector near the final target."""
    robot: RigidObject = env.scene[robot_cfg.name]
    object: RigidObject = env.scene[object_cfg.name]
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]
    command = env.command_manager.get_command(command_name)
    ...
    gripper_joint = robot.data.joint_pos[:, -1]
    gripper_opening = torch.clamp(gripper_joint / 0.04, 0.0, 1.0)
    opened_enough = (gripper_opening >= min_open_fraction).float()

    ee_pos_w = ee_frame.data.target_pos_w[..., 0, :]
    ee_object_distance = torch.norm(object_pos_w - ee_pos_w, dim=1)
    separation_reward = torch.clamp(ee_object_distance / max(separation_threshold, 1.0e-6), 0.0, 1.0)

    return stage_complete * well_positioned * opened_enough * gripper_opening * separation_reward
